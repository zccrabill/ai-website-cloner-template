 ...
 *   - website.intake_received → notify Zachariah (Sitecraft project brief)
...
// website.intake_received — a Sitecraft project brief came in via the
// /sitecraft landing form. Emails Zachariah the details with the visitor's
...
function websiteIntakeReceivedTemplate(payload: IncomingPayload): EmailSpec {
  ...
  const projectTypes = Array.isArray(d.project_types)
    ...
    : "—";
  const body = `
    <p style="color:#6B5B4E;font-size:14px;line-height:1.6;margin-bottom:20px;">A new Sitecraft project brief just came in via the /sitecraft landing page.</p>
    <div style="background:#FAF8F5;border:1px solid rgba(31,24,16,0.08);border-radius:12px;padding:20px;margin-bottom:16px;">
      ${row("Name", fullName)}
      ${row("Email", `<a href="mailto:${escapeHtml(senderEmail)}" style="color:#C17832;">${escapeHtml(senderEmail)}</a>`)}
      ${d.phone ? row("Phone", escapeHtml(d.phone)) : ""}
  ...
  return {
    ...
    to: ADMIN_EMAIL,
    reply_to: typeof senderEmail === "string" ? senderEmail : undefined,
    subject: `New Sitecraft brief: ${fullName} — ${senderEmail}`,
    html: shell("New Sitecraft project brief", body),
  };
}
